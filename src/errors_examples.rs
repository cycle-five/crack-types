...
pub fn with_context<T, E>(result: Result<T, E>, context: &'static str) -> Result<T, ErrorContext<E>> {
    result.map_err(|err| ErrorContext { context, source: err })
}

#[cfg(test)]
mod tests {
    use super::*;
    use crate::{CrackedError, verify};
